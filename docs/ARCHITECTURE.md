...
### ADR-14. 실행 품질 표면화 (item 9)
- **결정**: `run_workflow`가 로그·더미여부로 `run_status`(success/degraded/failed)·`failed_nodes`·`fallback_nodes`를 산출해 State·API·이력에 싣고, UI 배너와 DOCX 다운로드 경고로 노출(`workflow.py:_assess_quality`).
- **배경**: `_safe`/`complete_*`가 오류를 fallback으로 흡수해 완주하지만, 사용자는 결과가 정상 생성된 것으로 오해할 수 있음.
- **결과**: fallback/더미가 섞인 결과를 정직하게 경고(품질 투명성). 판정은 로그 문자열 기반의 휴리스틱이라 로그 포맷 변경에 의존적.

### ADR-15. 수정(/revise) 결과의 이력 반영·재평가 (item 4)
