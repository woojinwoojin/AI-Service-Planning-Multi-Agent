...
- **입력 자동완성** — 프로젝트명(+기존 입력)으로 **비어 있는 항목만** AI가 채움. 사용자가 이미 쓴 값은 보존·문맥으로만 활용하고, AI가 채운 필드는 `AI 추천` 배지로 표시(수정 시 `AI 추천 수정됨`)
...
- **회귀 테스트** — `pytest` 132개 (LLM 호출 없이 검증 로직·라우트 커버) · `ruff` 정적 검사 통과
...
tests/                      # pytest 132개 (LLM 호출 없이 검증 로직·라우트 테스트)
