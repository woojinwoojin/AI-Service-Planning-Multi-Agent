...
> 아이디어 한 줄을 입력하면 여러 AI Agent가 **시장조사 → 경쟁사 분석 → PESTEL → SWOT → 수익모델 → 리스크 → 기획서 작성 → 심사 → 출처 검증**을 순차 수행해 근거 있는 서비스 기획서를 만들어 주는 도구입니다.
...
- 마지막에 검증 Agent가 기획서 주장을 근거와 대조
- 사용자가 결과를 보고 **직접 수정 요청**(Human-in-the-Loop)으로 재작성 가능
- 최종 기획서를 **Markdown / JSON / Word(.docx)**로 저장·다운로드
...
사용자 입력
  ...
  → Verify Agent          (기획서 주장 ↔ 근거 대조, 지지율 산출)
  → 최종 기획서 + Agent별 산출물
```
...
| 기획서 일관성 | 16.3 | 15.8 |
| 근거와 출처 | 15.7 | 16.6 |
| **총점 (LLM 심판)** | **85.1** | **87.2 (+2.1)** |
| **검증 가능한 실제 출처 수 (객관)** | **0** | **5** |

- **Multi-Agent가 6주제 중 5개에서 우위**(1개 동점, 진 주제 없음).
- 정직한 관찰: **LLM 심판 점수만으로는 격차가 크지 않습니다.** 작은 LLM 심판은 "유창하지만 근거 없는 글"과 "실제 인용된 글"을 잘 구분하지 못합니다.
- **결정적 차이는 객관 지표**: Multi-Agent는 검증 가능한 실제 출처를 평균 5건 인용하고, 단일 LLM은 0건입니다(검색을 하지 않으므로). Multi-Agent의 가치는 점수가 아니라 **검증 가능한 근거**에 있습니다.
...
- **출처 인용** — 수집한 실제 URL을 기획서 `참고자료` 섹션에 명시
- **고정 14섹션 서식** — 프로젝트 개요 … PESTEL · SWOT · 수익 모델 … 위험요인 (PESTEL은 표로 렌더)
- **평가·자동 재작성** — Reviewer 5항목 평가, 총점 90 미만 시 1회 재작성
- **Human-in-the-Loop** — 사용자가 수정 요청을 넣어 재작성
- **산출물** — Markdown / 전체 결과 JSON / Word(.docx)
- **최소 웹 UI** — 입력 / 결과(Agent별) / 최종 기획서 3화면 (FastAPI가 서빙하는 자체완결 HTML)
- **단일 vs 멀티 비교 harness** — 재현 가능한 비교실험 (`run_compare.py`)
- **회귀 테스트** — `pytest` (LLM 호출 없이 검증 로직 커버)
...
| 산출물 | python-docx (.docx), Markdown, JSON |
| Frontend | 자체완결 HTML (인라인 CSS/JS, 외부 CDN 없음) |
| 테스트 | pytest |
| 상태 관리 | LangGraph State (별도 DB 없음) |
...
| Method | Endpoint | 설명 |
|---|---|---|
| GET | `/` | 최소 UI(입력/결과/최종 3화면) |
| GET | `/health` | 상태 · 더미 여부 · provider · 기본 모델 |
| GET | `/models` | 현재 provider에서 선택 가능한 모델 목록 |
| POST | `/run` | 아이디어 입력 → 전체 워크플로 실행, Agent별 결과 반환 |
| POST | `/revise` | Human-in-the-Loop 수동 수정요청 반영 재작성 |
| POST | `/run/save` | 실행 후 `.md` + `.json` + `.docx` 저장 |
...
# 4) 단일 vs Multi-Agent 비교실험 (선택)
python run_compare.py         # 결과: docs/comparison_result.md, outputs/comparison.json
...
app/
 ...
 ├─ graph/workflow.py       # LangGraph 워크플로 (노드·엣지·_safe)
 ├─ agents/
 ...
 │   ├─ risk.py             # 리스크
 │   ├─ draft_writer.py     # 기획서 작성 + 재작성
 │   ├─ reviewer.py         # 평가
 │   ├─ verifier.py         # 출처 검증
 │   └─ single_agent.py     # 단일 LLM 기준선(비교용)
 ├─ services/
 │   ├─ llm.py              # LLM 래퍼 (provider/모델·재시도·fallback)
 ...
 └─ static/index.html       # 최소 UI
tests/                      # pytest (LLM 호출 없이 검증 로직 테스트)
...
원래 12-Agent 풀버전 구상에서 다음은 **의도적으로 제외**했습니다(범위 관리): 별도 DB(SQLite)·RAG·PPTX·CI/CD·로그인. 현재는 LangGraph State + 파일 산출물로 충분히 동작합니다.

향후 후보: PPTX 산출, 프로젝트 이력 저장(DB), 섹션 간 연결성(일관성) 추가 개선, 다중 모델 비교. 자세한 배경은 [`ROADMAP.md`](ROADMAP.md) 참고.
