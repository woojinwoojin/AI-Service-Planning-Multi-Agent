...
> 🔒 **제출·발표 기준 코드 동결: `v1.0.0-submission`** (2026-07-28) · 테스트 665개 · 커버리지 96.77%
...
- **회귀 테스트** — `pytest` **665개** (LLM 호출 없이 검증 로직·라우트 커버) · `ruff` 정적 검사 통과 · CI 4게이트(ruff+pytest·gitleaks·pip-audit·docker build) · 커버리지 **96.77%**(하한 90%)
...
| 테스트 | pytest (665개) · ruff · GitHub Actions CI 4게이트 |
...
tests/                      # pytest 665개 (LLM 호출 없이 검증 로직·라우트 테스트)
...
- **본문 분량이 요건(A4 30~50쪽) 미달**입니다(실 LLM 실측 약 13.0쪽). 분량을 위한 분량은 넣지 않았습니다 —
