...
사용자가 '프로젝트명'만 입력했을 때(선택적으로 짧은 메모 포함), 뒤 기획 단계가 바로 쓸 수 있도록
나머지 입력 필드를 그럴듯하고 구체적인 초안으로 채웁니다. 사용자가 이후 자유롭게 수정합니다.

[작성 원칙]
- 프로젝트명에서 자연스럽게 도출되는 내용만 씁니다. 근거 없는 수치·고유명사는 지어내지 마세요.
- description: 어떤 서비스인지 1~2문장으로 명료하게.
- target_user: 핵심 목표 사용자를 한 문장으로 구체화(막연한 '모두' 금지).
- problem: 이 서비스가 해결하려는 사용자 문제를 한 문장으로.
- keywords: 도메인·기능·타깃을 아우르는 3~6개 핵심 키워드 배열.

[출력 형식]
다른 텍스트 없이 아래 JSON 하나만, 유효한 JSON 객체로 출력하세요. 모든 키를 포함합니다.
{"description": "", "target_user": "", "problem": "", "keywords": []}"""
