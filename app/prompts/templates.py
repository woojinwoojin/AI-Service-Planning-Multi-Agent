...
'사실 주장'에 한해 '제공된 근거(시장조사 결과·출처)'만을 기준으로 뒷받침 여부를 판정합니다.
...
- evidence_ids: 그 주장을 뒷받침하는 '근거 출처 목록'의 evidence_id(예: ev1, ev3)를 배열로 적으세요.
  · 특정 출처가 뒷받침하면 그 id 들을 적고, 특정 출처로 연결되지 않으면 빈 배열([])로 두세요.
  · 목록에 없는 id 를 지어내지 마세요. 출처 없이 '시장조사 결과' 등 분석 텍스트로만 지지되면 []로 둡니다.
