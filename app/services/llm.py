...
def dummy_fallback(dummy: dict) -> dict:
    ...
    `run_status=degraded` 와 로그의 '더미' 표기로 이미 정직하게 구분된다.

    더미 실행의 준수율은 **낮추지 않는다.** 구조 검사 관점에서는 실제로 채워져 있고, 낮추면
    '검사가 ok 로 갈 수 있는지'를 확인하는 배선 테스트가 무의미해진다. 대신
    `kosena.evaluate` 가 내용의 `[더미]` 표식을 보고 `data_source="dummy"` 와 경고 문구
    (`⚠️ 더미 데이터 기준(실 LLM 결과 아님)`)를 붙여 **실 LLM 성과와 구분한다** — 준수율 캡처가
    실행 성과처럼 읽히는 것을 막는 것이 목적이다.
    """
