...
def _sources(state: dict) -> str:
    ...
    정부·학술 자료를 위로 올려(`evidence.authority_rank`) 정량 주장의 근거가 어디서 왔는지
    먼저 보이게 한다 — **정렬**이고, 공식 출처를 따로 겨냥해 수집하는 것은 아니다.
    ...
    return "\n\n".join([
        ...
        "> 도메인 규칙으로 분류한 출처 유형에 따라 **정부·공공 → 학술 → 언론 → 커뮤니티 순으로 "
        "정렬**했다(우선 수집이 아니라 우선 정렬이다 — 검색은 공식기관 도메인을 따로 겨냥하지 "
        "않는다). 출처 유형은 **권위성 힌트일 뿐 신뢰도 판정이 아니며**, 기업 공식 사이트는 판단을 "
        "보류해 '기타'로 남는다.\n"
